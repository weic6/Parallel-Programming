...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 DimGrid(ceil(1.0*dim/BLOCKSIZE), 1, 1);
  dim3 DimBlock(BLOCKSIZE, 1, 1);
  spmvJDSKernel<<<DimGrid, DimBlock>>>(out, matColStart, matCols, matRowPerm, matRows,
  matData, vec, dim);
}
...
int main(int argc, char **argv) {
  ...
  hostOutput = (float *)malloc(sizeof(float) * dim);//!!! dim = size of output
